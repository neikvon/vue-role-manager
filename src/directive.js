...
function handler (el, binding) {
  ...
  if (typeof binding.value === 'function') {
    ...
    hasAccess = this.hasAccess(binding.value)
  }
  ...
  if (arg === 'action') {
    ...
    Object.keys(binding.modifiers).map(cls =>
      el.classList.contains(cls) ? null : (el.className += ' ' + cls)
    )
  } else {
    el.parentElement && el.parentElement.removeChild(el)
  }
